'use strict';
angular.module('appControllers').controller('ResultSync', ['$scope',

    function ($scope) {
        var resource = DataManager.dataResource.init(`${Config.Directories.OMR_CONFIG}\\resultsync.json`)
        $scope.init = () => {
        ...
        $scope.save = () => {
            ...
            test.once('finish', (data) => {
                ...
                bootbox.dialog({
                    title: 'Deseja salvar a configuração da <b>Sincronizador de Resultados</b>?',
                    message: message,
                    buttons: {
                        danger: {
                            label: 'Salvar',
