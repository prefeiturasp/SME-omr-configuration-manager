'use strict';
angular.module('appControllers').controller('FileOrganizer', ['$scope',

    function ($scope) {
        var resource = DataManager.dataResource.init(`${Config.Directories.OMR_CONFIG}\\fileorganizer.json`)
        $scope.init = () => {
        ...
        $scope.save = () => {
            ...
            test.once('finish', (data) => {
                ...
                bootbox.dialog({
                    title: 'Deseja salvar a configuração do <b>Organizador de Arquivos</b>?',
                    message: message,
                    buttons: {
                        danger: {
                            label: 'Salvar',
