'use strict';
angular.module('appControllers').controller('Processor', ['$scope',

    function ($scope) {
        var resource = DataManager.dataResource.init(`${Config.Directories.OMR_CONFIG}\\processor.json`)
        $scope.init = () => {
        ...
        $scope.save = () => {
            ...
            test.once('finish', (data) => {
                ...
                bootbox.dialog({
                    title: 'Deseja salvar a configuração do <b>Processador</b>?',
                    message: message,
                    buttons: {
                        danger: {
                            label: 'Salvar',
